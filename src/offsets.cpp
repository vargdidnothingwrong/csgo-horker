...
namespace Sig {
    ...
    SIGNATURE(ForceJump, "8b 15 ?? ?? ?? ?? 89 ?? 83 ?? ?? f6 ?? ?? 0f ?? ?? 44 ?? ?? c1 ?? 1e", 1);
...
bool Signatures::Find(Process &mem)
{
    ...
    uintptr_t forceJumpOffset = FindInClient(mem, ForceJump);
    Offset::Client::ForceJump = mem.GetCallAddress(forceJumpOffset);
    return true;
}
...
void Signatures::Print(Process &mem)
{
    ...
    PrintOffset(Client, ForceJump, sClient);
